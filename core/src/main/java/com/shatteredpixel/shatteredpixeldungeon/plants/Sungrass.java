...
public class Sungrass extends Plant {
	...
	public void activate( Character ch ) {
		...
		if (ch != null){
			if (ch instanceof Hero && ((Hero) ch).subClass == HeroSubClass.WARDEN) {
				Buff.affect(ch, Healing.class).setHeal(ch.healthMax, 0, 1);
			} else {
				Buff.affect(ch, Health.class).boost(ch.healthMax);
			}
		}
	...
	public static class Health extends Buff {
		...
		public boolean playGameTurn() {
			...
			partialHeal += (40 + target.healthMax)/150f;
			...
			if (partialHeal > 1){
				...
				if (target.healthPoints < target.healthMax) {
					...
					target.healthPoints += healThisTurn;
		...
		public float iconFadePercent() {
			return Math.max(0, (target.healthMax - level) / (float)target.healthMax);
		}
