...
public class QuickRecipe extends Component {
	...
	public QuickRecipe(Recipe r, ArrayList<Item> inputs, final Item output) {
		...
		for (final Item in : inputs) {
			...
			curr = new ItemSlot(in) {
				...
				protected void onClick() {
					JourneyPixelDungeon.scene().addToFront(new WndInfoItem(in));
				}
			};
		...
		if (hasInputs) {
			...
			if (!(JourneyPixelDungeon.scene() instanceof AlchemyScene)) {
				arrow.enable(false);
			}
		} else {
		...
		this.output = new ItemSlot(output){
			@Override
			protected void onClick() {
				JourneyPixelDungeon.scene().addToFront(new WndInfoItem(output));
			}
		};
		if (!hasInputs){
	...
	public class arrow extends IconButton {
		...
		protected void onClick() {
			...
			((AlchemyScene) JourneyPixelDungeon.scene()).populate(ingredients, Dungeon.hero.belongings);
		}
	...
	public static ArrayList<QuickRecipe> getRecipes( int pageIdx ){
		...
		switch (pageIdx){
			...
			case 1:
				...
				for (Class<?> cls : ItemGenerator.Category.SCROLL.classes){
			...
			case 3:
				...
				for (Class<?> cls : ItemGenerator.Category.POTION.classes){
