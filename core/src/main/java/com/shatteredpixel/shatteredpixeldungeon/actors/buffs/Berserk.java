...
public class Berserk extends Buff implements ActionIndicator.Action {
	...
	public boolean playGameTurn() {
		if (state == State.BERSERK){
			...
			if (powerLossBuffer > 0){
				...
				power -= GameMath.gate(0.1f, power, 1f) * 0.067f * Math.pow((target.healthPoints / (float) target.healthMax), 2);
	...
	private void startBerserking(){
		...
		float shieldMultiplier = 2f + 4*(float)Math.pow((1f-(target.healthPoints /(float)target.healthMax)), 3);
	...
	public void damage(int damage){
		...
		power = Math.min(maxPower, power + (damage/(float)target.healthMax)/3f );
