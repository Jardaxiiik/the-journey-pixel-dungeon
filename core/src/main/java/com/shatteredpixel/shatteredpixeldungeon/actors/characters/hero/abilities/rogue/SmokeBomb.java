...
public class SmokeBomb extends ArmorAbility {
		...
		if (target != null) {
			...
			if (!shadowStepping) {
				for (Mob mob : Dungeon.level.mobs.toArray(new Mob[0])) {
					if (Dungeon.level.adjacent(mob.position, hero.position) && mob.alignment != CharacterAlignment.ALLY) {
	...
	public static class NinjaLog extends NPC {

		{
			...
			healthPoints = healthMax = 20*Dungeon.hero.pointsInTalent(Talent.BODY_REPLACEMENT);
		}
