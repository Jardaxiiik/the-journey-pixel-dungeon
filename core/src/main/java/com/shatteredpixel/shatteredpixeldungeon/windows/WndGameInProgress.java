...
public class WndGameInProgress extends Window {
	...
	public WndGameInProgress(final int slot){
		...
		if (info.shield > 0)  statSlot( Messages.get(this, "health"), info.healthPoint + "+" + info.shield + "/" + info.ht );
		else                statSlot( Messages.get(this, "health"), (info.healthPoint) + "/" + info.ht );
		...
		RedButton cont = new RedButton(Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				JourneyPixelDungeon.switchScene(InterlevelScene.class);
			}
		};
		...
		RedButton erase = new RedButton( Messages.get(this, "erase")){
			...
			protected void onClick() {
				...
				JourneyPixelDungeon.scene().add(new WndOptions(Icons.get(Icons.WARNING),
