...
public class ToxicGasRoom extends SpecialRoom {
	...
	public void paint(Level level) {
		...
		for (Point p : getPoints()){
			...
			if (level.map[cell] == Terrain.EMPTY) {
				...
				Emitter.seed(cell, 30, ToxicGas.class, level);
			}
		}
		...
		for (int i = 0; i < traps; i++){
			...
			Emitter.seed(cell, 12, ToxicGasSeed.class, level);
		...
		level.dropItemOnPosition(mainGold, furthestPos).type = Heap.Type.SKELETON;

		for (int i = 0; i < 2; i++){
			level.dropItemOnPosition(new Gold().random(), goldPositions.remove(0)).type = Heap.Type.CHEST;
		}
	...
	public boolean canPlaceCharacter(Point p, Level l) {
		Emitter gas = l.blobs.get(ToxicGas.class);
		return gas == null || gas.volume == 0 || gas.cur[l.pointToCell(p)] == 0;
	}

	public static class ToxicGasSeed extends Emitter {
				...
				for (int j = area.left-1; j <= area.right; j++) {
					...
					if (Dungeon.level.insideMap(cell)) {
						...
						if (gas == null || gas.volume == 0){
							GameScene.add(Emitter.seed(cell, off[cell], ToxicGas.class));
