	...
	public static class EarthGuardian extends NPC {
		...
		public int getArmorPointsRolled() {
			...
			if (Dungeon.isChallenged(Challenges.NO_ARMOR)){
				return armorPoints + Random.NormalIntRange(wandLevel, 2 + wandLevel);
			} else {
				return armorPoints + Random.NormalIntRange(wandLevel, 3 + 3 * wandLevel);
			}
		}
		...
		private class Wandering extends Mob.Wandering{
			...
			public boolean playGameTurn(boolean enemyInFOV, boolean justAlerted) {
				if (!enemyInFOV){
					Buff.affect(Dungeon.hero, RockArmor.class).addArmor(wandLevel, healthPoints);
