...
import com.shatteredpixel.shatteredpixeldungeon.actors.characters.mobs.GoldenMimic;
...
public class CursedWand {
	...
	public static boolean cursedEffect(final Item origin, final Character user, final Character target){
		return cursedEffect(origin, user, target.position);
	}
	...
	private static boolean commonEffect(final Item origin, final Character user, final int targetPos){
		switch(Random.Int(4)){
			...
			case 2:
				if(Random.Int(2) == 0) {
					if (user != null && !user.getProperties().contains(Character.Property.IMMOVABLE)) {
		...
		switch(Random.Int(4)){
			...
			case 1:
				...
				if (target != null) {
					...
					toHeal.healthPoints = Math.min(toHeal.healthMax, toHeal.healthPoints + damage);
					...
					toDamage.receiveDamageFromSource(damage, new CursedWand());
			...
			//shock and recharge
			case 3:
				new ShockingTrap().set( user.position).activate();
	...
	private static boolean veryRareEffect(final Item origin, final Character user, final int targetPos){
		switch(Random.Int(4)){
			...
			case 2:
				...
				try {
					...
					if(Messages.lang() != Languages.ENGLISH){
						...
						JourneyPixelDungeon.runOnRenderThread(
								new Callback() {
									@Override
									public void call() {
										GameScene.show(
			...
			case 3:
				...
				Dungeon.level.dropItemOnPosition(result, user.position).sprite.drop();
				return true;
		}
	}
