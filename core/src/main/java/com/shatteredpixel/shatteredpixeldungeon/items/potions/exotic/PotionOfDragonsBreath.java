	...
	private CellSelector.Listener targeter = new CellSelector.Listener() {
		...
		public void onSelect(final Integer cell) {
			...
			if (cell == null && identifiedByUse){
				showingWindow = true;
				JourneyPixelDungeon.runOnRenderThread(new Callback() {
					@Override
					public void call() {
									...
									public void call() {
										...
										for (int cell : cone.cells){
											...
											if (Dungeon.level.adjacent(bolt.sourcePos, cell) && !Dungeon.level.flamable[cell]){
												...
												GameScene.addMob( ActorLoop.seed( cell, 5, Fire.class ) );
											}
										...
										for (int cell : adjacentCells){
											for (int i : PathFinder.OFFSETS_NEIGHBOURS4){
												if (Dungeon.level.trueDistance(cell+i, bolt.sourcePos) > Dungeon.level.trueDistance(cell, bolt.sourcePos)
													...
													GameScene.addMob( ActorLoop.seed( cell+i, 5, Fire.class ) );
												}
											}
										}
